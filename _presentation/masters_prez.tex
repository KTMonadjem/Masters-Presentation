...
%	\begin{block}{Avalanche victim rescue drone~\footcite{dickensheets2015}}
%		\begin{myitemize}
...
\begin{frame} \frametitle{New case study -- Sensor fusion}
	\begin{block}{Dealing with neural network input perturbations using sensor fusion}
		\begin{myitemize}
			\item Autonomous vehicle prediction system.
			\item Uses sensor fusion to increase prediction accuracy.
			\begin{myitemize}
				\item Done synchronously using Meta Neural Networks.
				\item Uses synchronous runtime enforcement.
				\item Formal and timing guarantees can be provided.
			\end{myitemize}
		\end{myitemize}
	\end{block}
\end{frame}
...
\begin{frame} \frametitle{New case study -- Sensor fusion}
	\begin{block}{The enforced policy in question}
		\begin{figure}[t]
			\centering
			\scalebox{0.8}{\input{fig/sign-rte.tex}}
...
\begin{frame} \frametitle{New case study -- Preliminary results}
	\begin{block}{Basic enforcer that enforcers MNN and LiDAR output}
		\begin{table}[]
			\centering
			\resizebox{\textwidth}{!}{%
				\begin{tabular}{|l|l|l|l|l|l|}
					\hline
					Class misclassified  & Person & Vehicle & Street sign & Nothing of relevance & Total \\ \hline
					\multicolumn{6}{|l|}{Number of misclassifications (\%)}                               \\ \hline
						Without the enforcer & 0.44   & 3.59    & 3.15        & 2.18                 & 9.36 \\ \hline
						With the enforcer    & 0.97   & 1.09    & 0.6        & 2.97                 & 5.63  \\ \hline
					\end{tabular}%
				}
				\caption{Table showing preliminary results of the AV prediction SNN using perfect images}
				\label{tbl:snn-res}
			\end{table}
	\end{block}
\end{frame}
...
\begin{frame} \frametitle{New case study -- Preliminary results}
	\begin{block}{Basic enforcer that enforcers MNN and LiDAR output, input images are perturbed}
		\begin{table}[]
			\centering
			\resizebox{\textwidth}{!}{%
				\begin{tabular}{|l|l|l|l|l|l|}
					...
					Class misclassified  & Person & Vehicle & Street sign & Nothing of relevance & Total \\ \hline
					\multicolumn{6}{|l|}{Number of misclassifications (\%)}                               \\ \hline
					Without the enforcer & 0.35 & 5.42    & 5.26 & 25.63  & 36.66 \\ \hline
					With the enforcer    & 0.9 & 1.07 & 0.65   & 23.15   & 25.77  \\ \hline
				\end{tabular}%
			}
			\caption{Table showing preliminary results of the AV prediction SNN using perturbed images}
			\label{tbl:snn-respert}
		\end{table}
	\end{block}
\end{frame}
...
\begin{frame}\frametitle{Future work -- Final Master's sections}
	\begin{block}{Import Keras ANNs into C + Esterel SNNs}
		\begin{myitemize}
			\item Compiler that takes the parsed Python class and generates SNNs.
			\begin{myitemize}
				\item In Python (Tensorflow and Keras in Python).
				\item SNNs in C.
				\item Synchronous semantics -- Esterel, Pret-C, other?
			\end{myitemize}
			\item Parse files and compile to SNN.
		\end{myitemize}
	\end{block}
	\begin{block}{Realistic AV model}
		\begin{myitemize}
			\item Breaking and lane changing.
			\item Physics incorporated.
		\end{myitemize}
	\end{block}
\end{frame}
